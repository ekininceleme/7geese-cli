...
type Client struct {
	...
	// SessionRefresher, when set, is called on 401 to attempt a silent re-auth.
	// Returns true if new credentials were saved; the client reloads config and retries once.
	SessionRefresher func(configPath string) bool
}
...
func New(cfg *config.Config, timeout time.Duration, rateLimit float64) *Client {
	...
	cacheDir := filepath.Join(homeDir, ".cache", "7geese-cli")
...
func (c *Client) do(method, path string, params map[string]string, body any, headerOverrides map[string]string) (json.RawMessage, int, error) {
	...
	for attempt := 0; attempt <= maxRetries; attempt++ {
		...
		if req.Header.Get("User-Agent") == "" {
			req.Header.Set("User-Agent", "7geese-cli/v1")
		}
		...
		// Session expired — try to refresh from browser once, then retry
		if resp.StatusCode == 401 && attempt == 0 && c.SessionRefresher != nil && c.Config != nil {
			if c.SessionRefresher(c.Config.Path) {
				if newCfg, loadErr := config.Load(c.Config.Path); loadErr == nil {
					c.Config = newCfg
