...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "7geese-cli",
		Short: `7geese-cli — export your 7Geese performance data as JSON.`,
		Long: `7geese-cli — export your 7Geese performance data as JSON.

Commands:
  auth login   Log in using your existing Chrome session — no API key needed.
  sync         Pull your data into a local database.
  me export    Export everything (objectives, 1:1s, recognition, reviews) to JSON.
  doctor       Check that auth and connectivity are working.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("7geese-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see '7geese-cli profile list')")
...
func (f *rootFlags) newClient() (*client.Client, error) {
	...
	c.DryRun = f.dryRun
	c.NoCache = f.noCache
	c.SessionRefresher = TryRefreshSession
	return c, nil
}
