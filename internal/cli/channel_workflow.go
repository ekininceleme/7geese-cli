...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  7geese-cli workflow archive

  # Full re-archive (ignore previous sync state)
  7geese-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"badges", "categories", "checkins", "feedbackrequest", "notifications", "objectivekeyresults", "objectives", "oneononenotes", "oneonones", "organizationalobjectives", "peer_feedback", "performancecycles", "recognitionbadges", "team", "teamobjectives", "user", "userprofile",  }
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  7geese-cli workflow status

  # Show status as JSON
  7geese-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("7geese-cli")
			}
