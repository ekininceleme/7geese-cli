...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".7geese-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("API_7GEESE_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("API_7GEESE_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "7geese-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.7geese-cli/feedback.jsonl.
When ` + "`API_7GEESE_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`API_7GEESE_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  7geese-cli feedback list
  7geese-cli feedback list --limit 5
  7geese-cli feedback list --json`,
