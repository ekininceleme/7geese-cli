...
In local mode: searches locally synced data only.`,
  ...
  7geese-cli search "error timeout"

  # Force local search only
  7geese-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  7geese-cli search "critical" --type transactions --data-source local

  # JSON output for piping
  7geese-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun '7geese-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
