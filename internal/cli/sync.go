...
Exit codes & warnings:
  ...
  7geese-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  7geese-cli sync --full

  # Incremental sync: only records from the last 7 days
  7geese-cli sync --since 7d

  # Parallel sync with 8 workers
  7geese-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  7geese-cli sync --latest-only`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			// Resolve current user's profile ID from /status/ so we can
			// inject server-side filters — no company-wide data downloaded.
			...
			for i := 0; i < concurrency; i++ {
				...
				go func() {
					defer wg.Done()
					for job := range work {
						res := syncResource(c, db, job.resource, sinceTS, full, maxPages, job.extraParams)
						results <- res
					}
				}()
			}

			// Enqueue all resources — all are session-scoped by the API.
			for _, resource := range resources {
				work <- syncJob{resource: resource, extraParams: map[string]string{}}
			}
			close(work)
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/7geese-cli/data.db)")
...
// extraParams are merged into every page request — use them for server-side
// user-scoping filters (e.g. target=, creator=) so no company-wide data lands.
func syncResource(c interface {
	Get(string, map[string]string) (json.RawMessage, error)
	RateLimit() float64
}, db *store.Store, resource, sinceTS string, full bool, maxPages int, extraParams map[string]string) syncResult {
	started := time.Now()
	...
	for {
		...
		// Inject user-scoping filters (e.g. target=, creator=)
...
// 7Geese uses DRF offset pagination: ?limit=N&offset=N.
...
func extractPageItems(data json.RawMessage, cursorParam string) ([]json.RawMessage, string, bool) {
	...
	// Try common item keys first (fast path). "objects" is the 7Geese/DRF key.
	itemKeys := []string{"objects", "data", "results", "items", "records", "nodes", "entries"}
...
// Handles 7Geese's DRF pattern: {"meta": {"next": "...?offset=N", "total_count": N}, "objects": [...]}.
func extractPaginationFromEnvelope(envelope map[string]json.RawMessage, cursorParam string) (string, bool) {
	var hasMore bool

	// 7Geese/DRF: meta.next is a URL; extract the offset from it.
	if rawMeta, ok := envelope["meta"]; ok {
		var meta map[string]json.RawMessage
		if json.Unmarshal(rawMeta, &meta) == nil {
			// meta.next is null when there are no more pages
			if rawNext, ok := meta["next"]; ok {
				var nextURL string
				if json.Unmarshal(rawNext, &nextURL) == nil && nextURL != "" {
					parsed, err := url.Parse(nextURL)
					if err == nil {
						if offsetStr := parsed.Query().Get("offset"); offsetStr != "" {
							return offsetStr, true
						}
					}
				}
			}
	...
	if nextCursor == "" {
		paginationWrapperKeys := []string{"response_metadata", "pagination", "paging"}
